...
func createArchiveLocationSecret(tmpl *wfv1.Template, volMap map[string]apiv1.Volume, uniqueKeyMap map[string]bool) {
	...
	if s3ArtRepo := tmpl.ArchiveLocation.S3; s3ArtRepo != nil {
		...
		createSecretVal(volMap, azureBlobRepo.AccountNameSecret, uniqueKeyMap)
		createSecretVal(volMap, azureBlobRepo.SecretKeySecret, uniqueKeyMap)
	}
}
...
func createSecretVolume(volMap map[string]apiv1.Volume, art wfv1.Artifact, keyMap map[string]bool) {
	if art.S3 != nil {
		...
		createSecretVal(volMap, art.AzureBlob.AccountNameSecret, keyMap)
		createSecretVal(volMap, art.AzureBlob.SecretKeySecret, keyMap)
	}
}
